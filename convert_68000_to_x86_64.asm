...
read_int:
    ...
    xor rcx, rcx         ; RCX = digit counter (to verify input was provided)
.read_char:
    ...
    inc rcx              ; Increment digit count (used to detect empty input)
    jmp .read_char       ; Loop to get next character

.done:
    test rcx, rcx        ; Did the user enter anything before the newline?
    jz .err              ; If not, it's an error
    mov rax, rbx         ; Move final result to return register RAX
    xor rdx, rdx         ; Set RDX to 0 (Success)
    ret
...
.flush:
    ; Logic: Clear the remaining characters in the stdin buffer after an error
    ; This prevents leftover characters from being read as the next number.
    cmp byte [char_buf], 10 ; Is the last character already a newline?
    je .exit_err
    mov rax, 0           ; sys_read
    mov rdi, 0           ; stdin
    mov rsi, char_buf
    mov rdx, 1
    syscall
    jmp .flush           ; Continue reading until we hit the newline
.exit_err:
    mov rdx, r10         ; Move the error code into RDX for the caller
    ret
...
handle_error_print:
    ...
    ret
.o: PRINT_STR overflow_msg ; Print specific overflow message
    ret
